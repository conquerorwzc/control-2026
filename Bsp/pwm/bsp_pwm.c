...
static uint32_t PWMSelectTclk(TIM_HandleTypeDef *htim)
{
  ...
  // APB1 domain timers
  if ((tclk_temp >= D2_APB1PERIPH_BASE) &&
      (tclk_temp < (D2_APB1PERIPH_BASE + 0x8000UL)))
  {
    // In STM32H7, need to check if we're in D2 or D3 domain
    // For TIM2-7, TIM12-14 (APB1 timers)
    return (HAL_RCC_GetPCLK1Freq() * ((RCC->D2CFGR & RCC_D2CFGR_D2PPRE1) == 0 ? 1 : 2));
  }
  // APB2 domain timers
  else if ((tclk_temp >= D2_APB2PERIPH_BASE) &&
           (tclk_temp < (D2_APB2PERIPH_BASE + 0x8000UL)))
  {
    // For TIM1, TIM8, TIM15-17 (APB2 timers)
    return (HAL_RCC_GetPCLK2Freq() * ((RCC->D2CFGR & RCC_D2CFGR_D2PPRE2) == 0 ? 1 : 2));
  }
