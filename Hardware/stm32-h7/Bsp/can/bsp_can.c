...
// 在CAN产生接收中断会遍历数组,选出can和rxid与发生中断的实例相同的那个,调用其回调函数
...
CANInstance* CANRegister(CAN_Init_Config_s* config) {
  if (!idx) {
    ...
    LOGINFO("[bsp_can] FDCAN Service Init");
  }
  if (idx >= CAN_MX_REGISTER_CNT)  // 超过最大实例数
  {
    while (1) LOGERROR("[bsp_can] FDCAN instance exceeded MAX num, consider balance the load of FDCAN bus");
  }
  ...
  CANAddFilter(instance);          // 添加FDCAN过滤器规则
...
uint8_t CANTransmit(CANInstance* _instance, float timeout) {
  ...
  while (HAL_FDCAN_GetTxFifoFreeLevel(_instance->can_handle) == 0)  // 等待TX FIFO有空闲空间
  {
    ...
    {
      LOGWARNING("[bsp_can] FDCAN TX FIFO full! failed to add msg to FIFO. Cnt [%d]", busy_count);
      busy_count++;
      return 0;
    }
  }
  ...
  if (length <= 8) {
    ...
    switch (length) {
      ...
      case 8:
        ...
        break;
      default:
        while (1) LOGERROR("[bsp_can] FDCAN DLC error! Invalid length for classic CAN");
    }
  } else if (length <= 64) {
    // CAN FD模式下的DLC映射
    if (length <= 12)
    ...
    while (1) LOGERROR("[bsp_can] FDCAN DLC error! Maximum length is 64 bytes");
  }
