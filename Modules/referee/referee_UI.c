...
void UIDelete(referee_id_t* _id, uint8_t Del_Operate, uint8_t Del_Layer) {
  ...
  UI_delete_data.frametail =
      get_CRC16_check_sum((uint8_t*)&UI_delete_data, LEN_HEADER + LEN_CMDID + temp_datalength, 0xFFFF);
  /* 填入0xFFFF,关于crc校验 */
...
void UIGraphRefresh(referee_id_t* _id, int cnt, ...) {
  ...
  uint8_t temp_datalength =
      LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head + UI_Operate_LEN_PerDraw * cnt + LEN_TAIL;  // 计算交互数据长度
  ...
  memcpy(buffer, (uint8_t*)&UI_GraphReFresh_data,
         LEN_HEADER + LEN_CMDID + Interactive_Data_LEN_Head);  // 将帧头、命令码、交互数据帧头三部分复制到缓存中

  for (uint8_t i = 0; i < cnt; i++)  // 发送交互数据的数据帧，并计算CRC16校验值
  {
    graphData = va_arg(
        ap,
        Graph_Data_t);  // 访问参数列表中的每个项,第二个参数是你要返回的参数的类型,在取值时需要将其强制转化为指定类型的变量
