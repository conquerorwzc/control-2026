...
static void MyUIRefresh(Referee_Interactive_info_t *interactive_data)
{
    ...
    {
        char mode_buf[20];
        uint8_t mode_color = UI_Color_Green;

        uint8_t arm_calibrating = 0;
        uint8_t lift_calibrating = 0;
        ...
        // 底盘抬升标定状态判断
        if (robotdata != NULL &&
            robotdata->chassis != NULL)
        {
            if (!(robotdata->chassis->cali_state.all_cali_done &&
                  robotdata->chassis->cali_state.is_max_calibrated))
            {
                lift_calibrating = 1;
            }
        }

        // 平级覆盖逻辑：ARM CALIB 优先级高于 LIFT CALIB
        if (arm_calibrating)
        {
            snprintf(mode_buf, sizeof(mode_buf), "%-10s", "ARM CALIB");
            mode_color = UI_Color_Yellow;
        }
        else if (lift_calibrating)
        {
            snprintf(mode_buf, sizeof(mode_buf), "%-10s", "LIFT CALIB");
            mode_color = UI_Color_Orange;
        }

        UICharDraw(&UI_engineer_mode_text[1], "rs1", UI_Graph_Change, 7, mode_color, 16, 2,
                   350, 860, mode_buf);
