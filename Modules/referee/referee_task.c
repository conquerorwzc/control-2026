...
void SentryTask() {
  ...
  sentry_interaction_data->frametail=get_CRC16_check_sum((uint8_t *)&sentry_interaction_data, LEN_HEADER + LEN_CMDID + LEN_receiver + LEN_sender + LEN_sentry_cmd, 0xFFFF);
  memcpy(buffer, (uint8_t *)&sentry_interaction_data, LEN_HEADER + LEN_CMDID + LEN_receiver + LEN_sender + LEN_sentry_cmd + LEN_TAIL);
