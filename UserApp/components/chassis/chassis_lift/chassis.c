...
ChassisInstance *ChassisInit(Chassis_Init_Config_s *chassis_init_config)
{
    ...
    lf_radius = sqrtf((half_track_width + center_gimbal_offset_x) * (half_track_width + center_gimbal_offset_x) +
                      (half_wheel_base - center_gimbal_offset_y) * (half_wheel_base - center_gimbal_offset_y)) * DEGREE_2_RAD;
    ...
    while (chassis->front_legs[0].motor->measure.real_current == 0) osDelay(10);
#endif
#if !DISABLE_LEG_FRONT_RIGHT
    while (chassis->front_legs[1].motor->measure.real_current == 0) osDelay(10);
#endif
...
/**
 * @brief 爬楼梯状态机 (融合梯形曲线、左右非对称解耦与后腿物理特调)
 */
void Climb_FSM()
{
    ...
    if (!is_legs_assembled && chassis->cali_state.all_cali_done && chassis->cali_state.is_max_calibrated)
    {
        ...
        float stroke_rear_l = fabsf(chassis->cali_state.max_angle[0] - chassis->cali_state.init_angle[0]) * MAX_CALI_SAFE_RATIO;
        float stroke_rear_r = fabsf(chassis->cali_state.max_angle[1] - chassis->cali_state.init_angle[1]) * MAX_CALI_SAFE_RATIO;
        ...
        LiftLeg_Init(&chassis->rear_legs[LEFT], &lift_speed_feedforward[0], 1, REAR_TOTAL_TIME_SEC,
                     REAR_ACCEL_TIME_SEC, stroke_rear_l, REAR_MOVING_MAX_OUT, REAR_STOP_MAX_OUT);
