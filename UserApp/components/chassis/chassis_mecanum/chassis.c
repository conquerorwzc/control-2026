...
ChassisInstance* ChassisInit(Chassis_Init_Config_s* chassis_init_config) {
  ...
  for (int i = 0; i < 4; i++) {
    chassis_init_config->wheel_motor_config[i].can_init_config.can_handle = &hcan1;
  ...
  chassis_init_config->wheel_motor_config[0].can_init_config.tx_id = 1;
  chassis_init_config->wheel_motor_config[0].controller_setting_init_config.motor_reverse_flag =
      MOTOR_DIRECTION_REVERSE;

  chassis_init_config->wheel_motor_config[1].can_init_config.tx_id = 4;
