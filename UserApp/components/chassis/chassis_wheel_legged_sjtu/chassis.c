...
static void LQR_K_Calc(float K[4][10], const float coef[40][6], float l_l, float l_r)
{
    for (int n = 0; n < 40; n++)
    {
        int row = n / 10;
        int col = n % 10;
        K[row][col] = coef[n][0] + coef[n][1] * l_l + coef[n][2] * l_r + coef[n][3] * l_l * l_l +
...
static void LegController(void)
{
    ...
    float f_inertial = 0.5f * chassis->param.body_mass * (l_avg / chassis->param.track_width) *
                       chassis->state_var.phi_d * chassis->state_var.x_b_d;
...
static uint8_t AllMotorsOnline(void)
{
    for (int i = 0; i < 2; i++)
    {
        if (!DaemonIsOnline(leg[i]->joint_motor[0]->daemon))
            return 0;
        if (!DaemonIsOnline(leg[i]->joint_motor[1]->daemon))
            return 0;
        if (!DaemonIsOnline(leg[i]->wheel_motor->daemon))
            return 0;
    }
    return 1;
}
