...
static void LegController(void) {
  ...
  float f_l_r =
      PIDCalculate(&chassis->length_PID[0], leg[0]->virtual_model.length, chassis->chassis_ctrl_cmd.leg_length);
  float f_l_l =
      PIDCalculate(&chassis->length_PID[1], leg[1]->virtual_model.length, chassis->chassis_ctrl_cmd.leg_length);
