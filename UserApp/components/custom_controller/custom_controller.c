...
CustomController_t* CustomControllerInit(CustomController_Init_Config_s* init_config)
{
    ...
    // 初始化电机数据
    for (int i = 0; i < 5; i++) {
        controller->motor_data[i].id = i + 1;  // 电机 ID 为 1, 2, 3, 4, 5
...
void CustomController_SendAllData(CustomController_t* controller)
{
    ...
    // ==============================================================
    // 关键修复 1：严格限制发送频率为最高 30Hz
    // 防止底层调用过于频繁导致图传链路被裁判系统限流丢包
    // ==============================================================
    static uint32_t last_send_time = 0;
    uint32_t current_time = HAL_GetTick(); // 获取当前系统毫秒级滴答
    if (current_time - last_send_time < 34) {
        return; // 距离上次发送不足 34ms，直接返回不处理
    }
    last_send_time = current_time;

    // ==============================================================
    // 关键修复 2：将数据段严格补齐到 30 字节
    // 官方 0x0302 协议严格要求 Data 段长度为 30
    // ==============================================================
    uint8_t controller_data[30] = {0};

    // 数据包类型标识
    controller_data[0] = 0x20;
    ...
    // 发送数据包，此时 sizeof(controller_data) 是 30 字节
