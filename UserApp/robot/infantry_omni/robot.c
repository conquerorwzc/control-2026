...
static void CalcOffsetAngle() {
  angle = (uint16_t)robot->gimbal->yaw_master_motor->measure.angle_single_round;
...
static void RemoteControlSet() {
  ...
  if (chassis_ctrl_cmd->chassis_mode == CHASSIS_FOLLOW) {
    chassis_ctrl_cmd->wz = (15.0f) * (float)rc_data[TEMP].rc.rocker_r_;  // 主动跟随量
  }
}
...
static void MouseKeySet() {
  ...
  if (gimbal_ctrl_cmd->gimbal_mode == GIMBAL_ON)
  {
    ...
    gimbal_ctrl_cmd->pitch -= (float)rc_data[TEMP].mouse.y * 0.003f; // 纵向灵敏度调节
  }
...
static void EmergencyHandler() {
  // 两switch都在下断电
  if ((switch_is_down(rc_data[TEMP].rc.switch_right) && switch_is_down(rc_data[TEMP].rc.switch_left))||switch_is_off(rc_data[TEMP].rc.switch_left)||switch_is_off(rc_data[TEMP].rc.switch_right))
  {
  ...
  if (switch_is_down(rc_data[TEMP].rc.switch_right)||switch_is_off(rc_data[TEMP].rc.switch_right))
  {
    chassis_ctrl_cmd->chassis_mode = CHASSIS_POWER_OFF;
  }
  else
  ...
  if (switch_is_down(rc_data[TEMP].rc.switch_left)||switch_is_off(rc_data[TEMP].rc.switch_left))
  {
    shoot_ctrl_cmd->shoot_mode = SHOOT_OFF;
    shoot_ctrl_cmd->friction_mode = FRICTION_OFF;
    shoot_ctrl_cmd->load_mode = LOAD_STOP;
...
void RobotInit() {
  ...
  *rc_data_last = *robot->rc_data;
  ...
  robot->gimbal = GimbalDualYawInit(&gimbal_dual_yaw_config);
  ...
  vision_recv_data=VisionInit(&gimbal_dual_yaw_config.imu_init_config);
