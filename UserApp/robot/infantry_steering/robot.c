...
float chaos_func(float x)
{
    ...
    float y = arm_cos_f32(t) + 0.5f * arm_cos_f32(3.0f * t) + 0.25f * arm_cos_f32(9.0f * t) +
              0.125f * arm_cos_f32(27.0f * t) + 0.0625f * arm_cos_f32(81.0f * t) + 0.03125f * arm_cos_f32(243.0f * t);
...
static void DualBoardCtrlSet()
{
    ...
    if (CANCommIsOnline(can_comm_instance))
    {
        ...
        //
        interactive_data->Chassis_Power_Data.chassis_power_mx =
            robot->chassis->chassis_ctrl_cmd.max_power; // 示例功率值
                                                        //
        ...
        // //interactive_data.autoaim_mode = robotdata->gimbal->gimbal_ctrl_cmd.gimbal_mode == GIMBAL_VISION ? 1 : 0; //
        // 自瞄模式(1为开启，0为关闭)
        interactive_data->autoaim_mode =
            cancomm_pack->gimbal_mode == GIMBAL_VISION ? 1 : 0; // 自瞄模式(1为开启，0为关闭)
        ...
        // interactive_data.bullet_left_real = referee_recv_info->ProjectileAllowance.projectile_allowance_17mm; //
        // 实体弹丸剩余

        // interactive_data.fric_speed_left = (uint16_t)robotdata->shoot->friction_motor[0]->measure.speed_aps; //
        // 左摩擦轮转速（取反使向上为正）
        interactive_data->fric_speed_left = cancomm_pack->friction_speed1; // 左摩擦轮转速（取反使向上为正）
        // interactive_data.fric_speed_right = (uint16_t)robotdata->shoot->friction_motor[1]->measure.speed_aps; //
        // 右摩擦轮转速
        interactive_data->fric_speed_right = cancomm_pack->friction_speed2; // 右摩擦轮转速

        interactive_data->chassis_relative_angle =
            robot->chassis->chassis_ctrl_cmd.offset_angle; // 底盘相对于云台的角度
...
RobotInstance *RobotInit()
{
    ...
    // 修改为对应串口,注意如果是自研板dbus协议串口需选用添加了反相器的那个
#elifdef STM32H723XX
