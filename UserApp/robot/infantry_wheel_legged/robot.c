...
void RobotInit() {
  ...
  // Enable 24V_R for test
  GPIO_Init_Config_s gpio_power_24v_l_config = {
      .GPIO_Pin = POWER_24V_L_Pin,
      .GPIOx = POWER_24V_L_GPIO_Port,
  };
  GPIOInstance* gpio_power_24v_l = GPIORegister(&gpio_power_24v_l_config);
  GPIOSet(gpio_power_24v_l);
  // Enable 24V_R for test
  GPIO_Init_Config_s gpio_power_24v_r_config = {
      .GPIO_Pin = POWER_24V_R_Pin,
      .GPIOx = POWER_24V_R_GPIO_Port,
  };
  GPIOInstance* gpio_power_24v_r = GPIORegister(&gpio_power_24v_r_config);
  GPIOSet(gpio_power_24v_r);
