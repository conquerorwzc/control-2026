...
static uint8_t joystick_fire = 0;   
static uint8_t joystick_burst = 0;  
#endif
...
// 统一的底盘运动解算层
// ==========================================
static void Chassis_Motion_Solve(RobotInstance* robot, Ctrl_Intent_s* intent) {
    ...
    switch (robot->robot_mode) {
        case ROBOT_CHASSIS_ROTATE: {
            rotate_frequency = 0.5f * intent->rotate_coff;
            rotate_omega = rotate_frequency * 2.0f * PI;
            chassis_ctrl_cmd->target_yaw += rotate_omega * robot->dt;

            float target_angle_to_gimbal = atan2f(intent->vy, intent->vx);
        ...
        case ROBOT_CHASSIS_FOLLOW: {
            ...
            if (input_mag > 0.0005f) {
                float move_angle_deg = (atan2f(intent->vy, intent->vx) - PI / 2.0f) * RAD_2_DEGREE;
...
#if defined(ONE_BOARD)
            chassis_ctrl_cmd->target_yaw += intent->yaw_delta;
            chassis_ctrl_cmd->wz = 0.0f;
#else
            chassis_ctrl_cmd->target_yaw = robot->chassis->imu->YawTotalAngle * DEGREE_2_RAD - robot->offset_angle * DEGREE_2_RAD;
            chassis_ctrl_cmd->wz = 0.0f;
#endif
            chassis_ctrl_cmd->vx = ramp_controller_update(&chassis_ramp, intent->vy, robot->chassis->state_var.x_b_d, robot->dt);
            chassis_ctrl_cmd->theta_ff = 0.0f;
            chassis_ctrl_cmd->roll = intent->roll_delta;
            chassis_ctrl_cmd->leg_length += intent->leg_length_delta;
            
            if (chassis_ctrl_cmd->leg_length > LEG_MAX_LENGTH) chassis_ctrl_cmd->leg_length = LEG_MAX_LENGTH;
            else if (chassis_ctrl_cmd->leg_length < LEG_MIN_LENGTH) chassis_ctrl_cmd->leg_length = LEG_MIN_LENGTH;
            break;
        }
        case ROBOT_CHASSIS_PROSTRATE_FOLLOW: {
            ...
            if (input_mag > max_speed) {
                intent->vx = intent->vx / input_mag * max_speed;
                intent->vy = intent->vy / input_mag * max_speed;
                input_mag = max_speed;
            }
        ...
        case ROBOT_CHASSIS_PROSTRATE_ROTATE: {
            ...
            if (input_mag > 5.0f) {
                float target_angle_to_gimbal_p = atan2f(intent->vy, intent->vx);
...
void JoyStickCtrl(RobotInstance* robot) {
  ...
  if (is_first_update) { rc_data_last = rc_data[TEMP]; is_first_update = 0; }
  
  Ctrl_Intent_s intent = {0};
  intent.rotate_coff = 1.0f;
  ...
  if (robot->robot_mode == ROBOT_CHASSIS_ROTATE) {
      ...
      intent.yaw_delta = (-0.25f) * (float)rc_data[TEMP].rc.rocker_r_ * robot->dt * DEGREE_2_RAD;
  ...
  Chassis_Motion_Solve(robot, &intent);
}
...
void MouseKeyCtrl(RobotInstance* robot) {
  ...
  static float speed_coff = 1.0f;      
  static float rotate_coff = 1.0f;     
  ...
  switch (rc_data[TEMP].key_count[KEY_PRESS_WITH_CTRL][Key_Z] % 3) {
    case 0: speed_coff = 1.5f; rotate_coff = 1.5f; break;
    case 1: speed_coff = 2.0f; rotate_coff = 2.0f; break;
    case 2: speed_coff = 2.5f; rotate_coff = 2.5f; break;
    default: break;
  }
  ...
  if (gimbal_ctrl_cmd->gimbal_mode == GIMBAL_ON) {
    ...
    if (gimbal_ctrl_cmd->pitch > PITCH_MAX_ANGLE) gimbal_ctrl_cmd->pitch = PITCH_MAX_ANGLE;
    else if (gimbal_ctrl_cmd->pitch < PITCH_MIN_ANGLE) gimbal_ctrl_cmd->pitch = PITCH_MIN_ANGLE;
  }
  ...
  if (rc_data[TEMP].key[KEY_PRESS].x != x_key_last) {
    if (rc_data[TEMP].key[KEY_PRESS].x) gimbal_ctrl_cmd->yaw += 180.0f;
  ...
  if (rc_data[TEMP].key[KEY_PRESS_WITH_CTRL].g != ctrl_g_key_last) {
    if (rc_data[TEMP].key[KEY_PRESS_WITH_CTRL].g) {
      if (chassis_ctrl_cmd->chassis_mode != CHASSIS_RECOVERY) chassis_ctrl_cmd->chassis_mode = CHASSIS_RECOVERY;
      else chassis_ctrl_cmd->chassis_mode = CHASSIS_ON;
    }
  ...
  if (rc_data[TEMP].key[KEY_PRESS].q) chassis_ctrl_cmd->roll = -0.4f;
  else if (rc_data[TEMP].key[KEY_PRESS].e) chassis_ctrl_cmd->roll = 0.4f;
  else chassis_ctrl_cmd->roll = 0.0f;

  if (rc_data[TEMP].key[KEY_PRESS_WITH_CTRL].r) chassis_ctrl_cmd->leg_length += 0.0000005f * 630.0f;
  else if (rc_data[TEMP].key[KEY_PRESS_WITH_CTRL].f) chassis_ctrl_cmd->leg_length -= 0.0000005f * 630.0f;
  ...
  Ctrl_Intent_s intent = {0};
  intent.rotate_coff = rotate_coff;
  
  if (rc_data[TEMP].key[KEY_PRESS].w) intent.vy = 0.5f * speed_coff;
  else if (rc_data[TEMP].key[KEY_PRESS].s) intent.vy = -0.5f * speed_coff;
  else intent.vy = 0.0f;

  if (rc_data[TEMP].key[KEY_PRESS].d) intent.vx = 0.5f * speed_coff;
  else if (rc_data[TEMP].key[KEY_PRESS].a) intent.vx = -0.5f * speed_coff;
  else intent.vx = 0.0f;

  if (robot->robot_mode == ROBOT_CHASSIS_FREE) {
      if (rc_data[TEMP].key[KEY_PRESS].w) intent.vy = (0.99f) * speed_coff;
      else if (rc_data[TEMP].key[KEY_PRESS].s) intent.vy = (-0.99f) * speed_coff;
      else intent.vy = 0.0f;
  }
...
void JoyStickCtrl(RobotInstance* robot) {
  ...
  if (switch_right(rc_data->rc.mode_switch)) {
    ...
    if (rc_data->rc.trigger == 1) {
      if (DWT_GetTimeline_s() - trigger_time > 1.0f) {
        joystick_fire = 1; joystick_burst = 1;
      } else {
        joystick_fire = 1; joystick_burst = 0;
      }
    } else {
      joystick_fire = 0; joystick_burst = 0;
    }
  }
  ...
  if (robot->robot_mode == ROBOT_CHASSIS_ROTATE) {
      // Original logic has no input for chassis_vx in JoyStickCtrl RC_ROTATE for OCD_CTRL
  } else if (robot->robot_mode == ROBOT_CHASSIS_FOLLOW) {
      intent.vx = 0.003f * (float)rc_data->rc.rocker_l_;
      intent.vy = 0.003f * (float)rc_data->rc.rocker_l1;
  } else if (robot->robot_mode == ROBOT_CHASSIS_FREE) {
      intent.yaw_delta = (-0.25f) * (float)rc_data->rc.rocker_r_ * robot->dt * DEGREE_2_RAD;
      ...
      intent.vy = rc_data->rc.rocker_l1;
      intent.vx = rc_data->rc.rocker_l_;
  }
...
void MouseKeyCtrl(RobotInstance* robot) {
  ...
  if (is_first_update) { rc_data_last = rc_data; is_first_update = 0; }
  ...
  if (rc_data->mouse_key.keyboard.x != x_key_last) {
    if (rc_data->mouse_key.keyboard.x) gimbal_ctrl_cmd->yaw += 180.0f;
  ...
  if (rc_data->mouse_key.keyboard.w) intent.vy = 0.5f * speed_coff;
  else if (rc_data->mouse_key.keyboard.s) intent.vy = -0.5f * speed_coff;
  else intent.vy = 0.0f;

  if (rc_data->mouse_key.keyboard.d) intent.vx = 0.5f * speed_coff;
  else if (rc_data->mouse_key.keyboard.a) intent.vx = -0.5f * speed_coff;
  else intent.vx = 0.0f;

  if (robot->robot_mode == ROBOT_CHASSIS_FREE) {
      if (rc_data->mouse_key.keyboard.w) intent.vy = (0.99f) * speed_coff;
      else if (rc_data->mouse_key.keyboard.s) intent.vy = (-0.99f) * speed_coff;
      else intent.vy = 0.0f;
  }
