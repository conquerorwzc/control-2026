...
// 统一的底盘运动解算层
// ==========================================
static void ChassisMotionSolve(RobotInstance* robot, Ctrl_Intent_s* intent) {
...
static void GimbalMotionSolve(RobotInstance* robot, Ctrl_Intent_s* intent) {
  Gimbal_Ctrl_Cmd_s* gimbal_ctrl_cmd = &robot->gimbal->gimbal_ctrl_cmd;

  if (intent->use_absolute_angle) {
    gimbal_ctrl_cmd->gimbal_mode = GIMBAL_VISION;
    gimbal_ctrl_cmd->yaw = intent->absolute_yaw;
    gimbal_ctrl_cmd->pitch = intent->absolute_pitch;
    ...
    gimbal_ctrl_cmd->yaw += intent->yaw_delta;
    gimbal_ctrl_cmd->pitch += intent->pitch_delta;
...
static void ShootMotionSolve(RobotInstance* robot, Ctrl_Intent_s* intent) {
  Shoot_Ctrl_Cmd_s* shoot_ctrl_cmd = &robot->shoot->shoot_ctrl_cmd;
  if (intent->shoot_flag == 1) {
    shoot_ctrl_cmd->load_mode = LOAD_1_BULLET;
  } else if (intent->shoot_flag == 2) {
...
void JoyStickCtrl(RobotInstance* robot) {
  ...
  intent.yaw_delta = -0.00035f * (float)rc_data[TEMP].rc.rocker_r_;
  intent.pitch_delta = -0.00006f * (float)rc_data[TEMP].rc.rocker_r1;
  ...
  ChassisMotionSolve(robot, &intent);
  GimbalMotionSolve(robot, &intent);
  ShootMotionSolve(robot, &intent);
  rc_data_last = rc_data[TEMP];
}
...
void MouseKeyCtrl(RobotInstance* robot) {
  ...
  if (rc_data[TEMP].mouse.press_r) {
    if (has_non_zero_data(vision_recv_data)) {
      intent.use_absolute_angle = 1;
      intent.absolute_yaw = vision_recv_data->gimbal_receive.yaw;
      intent.absolute_pitch = vision_recv_data->gimbal_receive.pitch;
    }
  }
  ...
  if (rc_data[TEMP].mouse.press_l) {
    if (shoot_ctrl_cmd->friction_mode == FRICTION_ON &&
      ...
      if (DWT_GetTimeline_s() - trigger_time > 0.2f) {
        intent.shoot_flag = 2;  // Burst
      } else {
        intent.shoot_flag = 1;  // Single
      }
    }
  } else {
    if (!switch_is_up(rc_data[TEMP].rc.switch_left)) {
  ...
  intent.yaw_delta = -(float)rc_data[TEMP].mouse.x * 0.002f;
  intent.pitch_delta = -(float)rc_data[TEMP].mouse.y * 0.002f;
...
void JoyStickCtrl(RobotInstance* robot) {
  ...
  intent.yaw_delta = -0.00035f * (float)rc_data->rc.rocker_r_;
  intent.pitch_delta = -0.00006f * (float)rc_data->rc.rocker_r1;
...
void MouseKeyCtrl(RobotInstance* robot) {
  ...
  intent.yaw_delta = -(float)rc_data->mouse_key.mouse.x * 0.002f;
  intent.pitch_delta = -(float)rc_data->mouse_key.mouse.y * 0.002f;
