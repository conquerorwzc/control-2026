...
static float WrapAngleDeg180(float angle);
static float CalcFollowTargetYawFeedforward(float yaw_delta);
static void PIDSwitchConfig(PIDInstance *pid, const PID_Init_Config_s *config);

/* Private user code ---------------------------------------------------------*/

/**
 * @brief 将角度归一化到 [-180, 180]，供旧 SJTU 跟随逻辑使用。
 *
...
/**
 * @brief 保留旧跟随前馈接口；当前 27demo 尚未接入云台耦合前馈。
 *
 * @param yaw_delta 云台本周期增量。
 * @return 固定返回 0，防止未标定的前馈写入底盘命令。
 */
static float CalcFollowTargetYawFeedforward(float yaw_delta) {
  (void)yaw_delta;
  return 0.0f;
...
/**
 * @brief 在参数实际变化时重置并切换项目通用 PID 配置。
 *
 * @param pid 待更新的 PID 实例。
 * @param config 目标初始化配置。
 */
static void PIDSwitchConfig(PIDInstance *pid, const PID_Init_Config_s *config) {
  PID_Init_Config_s mutable_config;

  if (pid == NULL || config == NULL) return;
  if (pid->Kp != config->Kp || pid->Ki != config->Ki || pid->Kd != config->Kd ||
      pid->MaxOut != config->MaxOut || pid->DeadBand != config->DeadBand || pid->Improve != config->Improve ||
      pid->IntegralLimit != config->IntegralLimit) {
    /* 项目现有 PIDInit 未声明 const；复制后再传入，避免修改原始配置。 */
    mutable_config = *config;
    PIDInit(pid, &mutable_config);
  }
}
...
static void RobotMotionSolve(RobotInstance* robot, Ctrl_Intent_s* intent) {
  ...
  switch (robot->robot_mode) {
    ...
    case ROBOT_CHASSIS_FOLLOW: {
      ...
      float follow_err = WrapAngleDeg180(move_angle_deg - robot->offset_angle);
      float rear_err = WrapAngleDeg180(follow_err - 180.0f);
...
#else
      float follow_err = WrapAngleDeg180(-robot->offset_angle);
      float rear_err = WrapAngleDeg180(follow_err - 180.0f);
      if (intent->reverse_follow || fabsf(rear_err) < fabsf(follow_err)) {
        follow_err = rear_err;
        input_vy = -input_vy;
  ...
  if (robot->robot_mode == ROBOT_CHASSIS_ROTATE || robot->robot_mode == ROBOT_CHASSIS_PROSTRATE_ROTATE) {
    gimbal_ctrl_cmd->chassis_rotate_wz = -0.2f * robot->chassis->imu->Gyro[2];
  } else {
