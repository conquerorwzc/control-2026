...
void VOFATask() {
  ...
  /* 27demo 不复用 M3508 功率模型；发布 ACE/LQR 安全状态供 VOFA 观察。 */
  visualized_data[0] = robot->chassis->state_var.x_b;
  visualized_data[1] = robot->chassis->state_var.x_b_d;
  visualized_data[2] = robot->chassis->state_var.theta_l;
  visualized_data[3] = robot->chassis->state_var.theta_r;
  visualized_data[4] = robot->chassis->state_var.theta_b;
  visualized_data[5] = (float)robot->chassis->core.output_block_reason;
  visualized_data[6] = (float)robot->chassis->core.motor_enabled;
...
void RobotTask() {
  ...
  WheelLeggedSjtu27ChassisTask(robot->chassis);
