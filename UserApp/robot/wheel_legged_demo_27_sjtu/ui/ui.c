 ...
 */
static uint8_t CalculateLegPosture(const WheelLeggedLegInstance_t *leg, float model_x[5], float model_y[5],
                                   float leg_phi[4]) {
  if (leg == NULL || leg->kinematics.forward_kinematics_status != PARALLEL_LEG_OK) {
    return 0;
  }

  const ParallelLegState_t *state = &leg->kinematics.state;
  model_x[0] = state->o1.x;
  model_y[0] = state->o1.y;
  model_x[1] = state->o2.x;
  model_y[1] = state->o2.y;
  model_x[2] = state->virtual_first_end_d.x;
  model_y[2] = state->virtual_first_end_d.y;
  model_x[3] = state->virtual_end_c.x;
  model_y[3] = state->virtual_end_c.y;
  model_x[4] = state->virtual_second_end_e.x;
  model_y[4] = state->virtual_second_end_e.y;
  leg_phi[0] = leg->kinematics.phi[LEG_KINEMATICS_INPUT_PHI1];
  leg_phi[1] = 0.0f;
  leg_phi[2] = 0.0f;
  leg_phi[3] = leg->kinematics.phi[LEG_KINEMATICS_INPUT_PHI2];
  return isfinite(model_x[0]) && isfinite(model_y[0]) && isfinite(model_x[1]) && isfinite(model_y[1]) &&
         isfinite(model_x[2]) && isfinite(model_y[2]) && isfinite(model_x[3]) && isfinite(model_y[3]) &&
         isfinite(model_x[4]) && isfinite(model_y[4]);
}
...
static void DrawSingleLegPosture(const WheelLeggedLegInstance_t *leg, uint8_t ui_index, int32_t base_y, uint32_t operate) {
...
static void DrawLegLengthValue(const WheelLeggedLegInstance_t *leg, uint8_t ui_index, int32_t base_y, uint32_t operate) {
  char name[4];
  ...
  if (leg == NULL) {
    ...
    int32_t length_mm = RoundToInt(leg->kinematics.state.length * 1000.0f);
...
static void DrawLegLengthValues(RobotInstance *robot, uint32_t operate) {
  const WheelLeggedLegInstance_t *top_leg = NULL;
  const WheelLeggedLegInstance_t *bottom_leg = NULL;
