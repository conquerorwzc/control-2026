...
`build_symbolic_model.m` 以机体、左右腿、左右轮、Yaw 和等法向力的 17 条原始 Newton-Euler 方程为起点。程序先消去 12 个接触内力，再以左右独立的纯轮式约束消去轮角加速度，整理为：
...
参考 SJTU/WBR 推导中的 `T_wl_to_l`、`T_wr_to_r` 表示轮对腿的内部反作用扭矩，不是本项目的电机 Tw。当前符号模型显式使用：

$$
T_{\mathrm{wheel\to leg}}=-T_w.
$$

轮转动方程使用电机 Tw，腿转动方程使用相反的定子反作用扭矩；不要把参考文档中的内部扭矩变量直接接到 H6215。
