...
%
% 动力学边界:
%   LQR 的 s_dot 是纯轮式平均速度；髋点诊断速度可包含 l_dot。单个 LQR
%   工作点采用固定腿长模型，不把 l_dot、l_ddot 作为动力学状态或输入。
...
fprintf('  原文模型输入顺序: [%s]\n', strjoin(parameter.model_input_order, ', '));
fprintf('  MCU 输入别名顺序: [%s]\n', strjoin(parameter.input_order, ', '));
fprintf('  MCU 输入符号: [%g, %g, %g, %g]\n', parameter.input_sign);
fprintf('  Tp_R/Tp_L 与 VMC 同向；Tw_R/Tw_L 正值定义为对应轮向前，实际电机方向仍需标定。\n\n');
...
fprintf('Step 2: 计算固定 %.3f m 工作点详情...\n', parameter.fixed_leg_length);
...
export_lqr_response_markdown('double_closed_loop_lqr_response.md', parameter, A_nominal, B_nominal, K_nominal);
...
fprintf('  输出: double_closed_loop_lqr_response.md\n');
...
% model 的 B_control 使用文章输入顺序；此处置换并映射到 MCU 输入顺序和正方向。
M = model.M_func(x, values);
B_model = model.B_control_func(x, values);
g = model.g_func(x, values);
B_mcu = B_model * parameter.input_map_model_from_mcu * diag(parameter.input_sign);
end
...
fprintf(file_id, '/* u_model (SJTU): [T_lw_L, T_lw_R, T_bl_L, T_bl_R]. */\n');
fprintf(file_id, '/* u_mcu = u0 - K * (x - x_ref). */\n');
fprintf(file_id, '/* Model contract: Tp=T_bl (body to leg), Tw=T_lw (motor/leg to wheel). */\n');
...
fprintf(file_id, '/* Fixed %.3f m design point. The current MCU LQR does not gate on leg length. */\n', ...
    parameter.fixed_leg_length);
...
function export_lqr_response_markdown(path, parameter, A, B, K)
% 由当前 A、B、K 自动导出正输入与正状态误差的第一拍响应，避免手写符号表失效。
file_id = fopen(path, 'w');
if file_id < 0
    error('Cannot open %s for writing.', path);
end
acceleration_rows = [2, 4, 6, 8, 10];
state_units = {'m', 'm/s', 'rad', 'rad/s', 'rad', 'rad/s', 'rad', 'rad/s', 'rad', 'rad/s'};

fprintf(file_id, '# 双闭环 LQR 正方向与第一拍响应合同\n\n');
fprintf(file_id, '> 本文件由 `compute_lqr_and_export.m` 自动生成，禁止手改。每次修改模型、参数或 Q/R 后重新生成。\n\n');
fprintf(file_id, '控制律为 `u_mcu=u0-K*(x-x_ref)`，输入顺序为 `[Tp_R,Tp_L,Tw_R,Tw_L]`。');
fprintf(file_id, '下表是未经过 MCU 力矩限幅和变化率限制的线性模型结果；真机方向仍须单轮、单腿低幅验证。\n\n');

fprintf(file_id, '## 正输入的直接加速度通道\n\n');
fprintf(file_id, '每行只施加对应的 `+1 N*m` 输入，表中数值来自线性化矩阵 B。正 Tw 定义为对应轮向前。\n\n');
fprintf(file_id, '| 正输入 | s_ddot | phi_ddot | theta_L_ddot | theta_R_ddot | theta_b_ddot |\n');
fprintf(file_id, '| --- | ---: | ---: | ---: | ---: | ---: |\n');
for input_index = 1:numel(parameter.input_order)
    acceleration = B(acceleration_rows, input_index);
    fprintf(file_id, '| %s | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g |\n', ...
        parameter.input_order{input_index}, acceleration);
end

fprintf(file_id, '\n## 单个正状态误差的 LQR 第一拍\n\n');
fprintf(file_id, '每行只令该状态误差等于 Bryson 表中的正边界 e_max，其余状态误差为零。');
fprintf(file_id, '四路输出为 `-K*e`；五个加速度来自 `(A-B*K)*e`，因此同时包含自然动力学和控制作用。');
fprintf(file_id, '正误差的幅值改变只会同比例缩放未限幅结果，不改变本表符号。\n\n');
fprintf(file_id, '| 正状态误差 | e_max | Tp_R | Tp_L | Tw_R | Tw_L | s_ddot | phi_ddot | theta_L_ddot | theta_R_ddot | theta_b_ddot |\n');
fprintf(file_id, '| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |\n');
closed_loop_matrix = A - B * K;
for state_index = 1:numel(parameter.state_order)
    state_error = zeros(numel(parameter.state_order), 1);
    state_error(state_index) = parameter.bryson_state_error_limit(state_index);
    output = -K * state_error;
    state_derivative = closed_loop_matrix * state_error;
    acceleration = state_derivative(acceleration_rows);
    fprintf(file_id, '| %s > 0 | %.6g %s | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g | %+.6g |\n', ...
        parameter.state_order{state_index}, parameter.bryson_state_error_limit(state_index), ...
        state_units{state_index}, output, acceleration);
end

fprintf(file_id, '\n## 如何解释这两张表\n\n');
fprintf(file_id, '- 正输入表回答“某一路执行器正命令在模型中直接把哪些自由度往哪边加速”。\n');
fprintf(file_id, '- 状态误差表回答“完整 LQR 面对单个正误差时第一拍怎样联合使用四路输入”。\n');
fprintf(file_id, '- 第一拍某一路输入与误差同向不代表正反馈；轮腿平衡包含非最小相位和多输入耦合，必须结合 `(A-BK)` 的时间响应判断。\n');
fprintf(file_id, '- 表中输出尚未经过 Tp/Tw 限幅。发生饱和后不再等于线性 LQR，必须另做限幅仿真和保护架验收。\n');
fprintf(file_id, '- H6215、关节传动、IMU 和编码器的实物正方向不能由本表证明。\n');
fclose(file_id);
end
